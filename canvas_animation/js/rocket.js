var diwali = (function() {
    ...
    Formation.prototype.addPlane = function(formation_x, formation_y, color) {
        var bomb_x = this.x + formation_x, bomb_y = this.y + formation_y;
        var bomb = new Rocket(resources.jet_img, bomb_x, bomb_y, color, formation_x, formation_y);
    ...
    Formation.prototype.travelTo = function(x, y, v) {
        ...
        for (var i in this.bombs) {
            f_bomb = this.bombs[i];
            f_bomb.travelTo(f_bomb.formation_x + x, f_bomb.formation_y + y, v)
        }
    };
    ...
    var Rocket = function(img, initX, initY, smoke_rgb, formation_x, formation_y) {
        this.img = img
        this.x = initX;
        this.y = initY;
    ...
    Rocket.prototype.travelTo = function(_x, _y, v) {
    ...
    Rocket.prototype.tick = function(ctx) {
        ...
        if (this.smoke_particles_list.length < 100 && !this.destroy_bomb) {
            var delta_x = (10 + 2 * Math.random()) * Math.cos(angle) - (80 + 4 * Math.random()) * Math.sin(angle);
            var delta_y = (10 + 2 * Math.random()) * Math.sin(angle) + (80 + 4 * Math.random()) * Math.cos(angle);
        ...
        var bbw = 150; //bounding box width
        if (this.x > ctx.canvas.width + bbw || this.y > ctx.canvas.height + bbw || this.x < 0 - bbw || this.y < 0 - bbw) {
            this.destroy_bomb = true;
        } else {
            this.destroy_bomb = false;
        }

        if (this.destroy_bomb && this.smoke_particles_list.length == 0) {
            this.destroy()
        }
    ...
    var destroy = function() {
        stage.destroy();
        setTimeout(function() {
            document.body.removeChild(document.getElementById("diwali_anim"));
        }, 50);
        delete stage;
    }
